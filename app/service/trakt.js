...
module.exports = class Trakt {
    ...
    _device_code(str, type) {
        ...
        this._debug(req);
        return fetch(req.url, req).then(async (response) => {
            if (response.status === 400) {
                throw { response };
            }
            const json = await response.json();
            return this._sanitize(json);
        }).catch(error => {
            throw (error.response && error.response.statusCode == 401) ? Error(error.response.headers['www-authenticate']) : error;
        });
    }
    ...
    _call(method, params) {
        ...
        this._debug(req);
        return fetch(req.url, req).then(async (response) => await this._parseResponse(method, params, response));
    }

    // Parse trakt response: pagination & stuff
    async _parseResponse (method, params, response) {
